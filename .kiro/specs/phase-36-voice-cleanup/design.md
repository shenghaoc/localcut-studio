...
3. **Loudness normalisation.** EBU R128 integrated-loudness measurement is an offline pass over the export track. We compute LUFS via the standard K-weighted filter + 400 ms gated blocks (`vDSP` for the IIR filtering). The normalisation gain stage applies a single dB delta to hit the target exactly once: loudness-only preview/export carries the gain in `AVAudioMix`, DSP-active preview/export applies it inside `VoiceCleanupDSP` after denoise/gate/compression and before the limiter, and measurement builds exclude any already-applied gain so repeat analysis measures the source signal. Changing an already-active gain rebuilds the audio mix. Preview route changes cancel scheduled DSP buffers and restore mixer unity before installing the next route, preventing stale overlap or double gain.
