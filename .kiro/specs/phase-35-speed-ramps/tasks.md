...
> Status: **In progress**. Core source-domain speed model, composition retiming,
> pitch-preserving audio-mix wiring, inspector controls, persistence, and unit /
> integration coverage are implemented. Bezier handle UI, source sample-time
> snapping, affected-range-only cache invalidation, and preview/export parity
> smoke remain follow-ups.
