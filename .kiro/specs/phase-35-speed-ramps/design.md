...
2. **Video pipeline.** Split the clip at every keyframe into segments. For each segment compute the source duration / output duration ratio. **`scaleTimeRange(_:toDuration:)` shifts every clip placed after it on the same track**, so the build order matters: for ramped clips we either (a) apply `scaleTimeRange` immediately after each segment insert and BEFORE inserting any subsequent clip on that track, or (b) isolate the ramped clip onto its own dedicated composition track. The current `CompositionBuilder.build(project:)` packs multiple non-overlapping clips per track for instruction-count efficiency; ramped clips trip the in-place-scale path explicitly, marked so future builder changes don't accidentally regress to "pack everything first, scale later". Continuous easing curves between keyframes are approximated by N sub-segments (default N=10 per keyframe pair; tunable based on perceived smoothness vs. instruction count cost).
