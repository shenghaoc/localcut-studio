...
> Status: **Implemented**. Target tag: **v0.1.4**.
...
2. **Video pipeline.** Split the clip at every keyframe into segments. For each segment compute the source duration / output duration ratio. **`scaleTimeRange(_:toDuration:)` shifts every clip placed after it on the same track**, and the in-place "apply immediately after each insert" workaround is fragile under cumulative rounding when multiple ramped clips share a track. The primary path is therefore **(b) isolate every ramped clip onto its own dedicated composition track**, leaving the per-track pack-many-clips optimisation in `CompositionBuilder.build(project:)` untouched. The in-place-scale variant (a) is documented only as a follow-up optimisation if the track count becomes a measurable instruction-count problem; it would carry its own rounding-tolerance tests and only kick in for single-ramp tracks. Continuous easing curves between keyframes are approximated by N sub-segments (default N=10 per keyframe pair; tunable based on perceived smoothness vs. instruction count cost). Internal segment boundaries snap to the source track's nearest sample duration, preserving the outer source range and original output ranges; VFR / fractional-boundary drift is therefore bounded to ±1 source frame at keyframe and sub-segment cuts.
