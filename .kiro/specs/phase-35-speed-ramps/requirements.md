...
- **R2.1** Video segments insert with `AVMutableCompositionTrack.scaleTimeRange(_:toDuration:)` per keyframe pair (per-track, not the composition-wide variant). Every ramped clip lands on its OWN dedicated composition track to avoid cumulative-rounding desync from `scaleTimeRange`'s shift-everything-after behaviour. Eased curves approximate via ≥10 sub-segments per pair.
