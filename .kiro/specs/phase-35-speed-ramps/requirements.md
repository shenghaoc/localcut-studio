...
- **R2.1** Video segments insert with `AVMutableCompositionTrack.scaleTimeRange(_:toDuration:)` per keyframe pair (per-track, not the composition-wide variant), applied immediately after each segment insert and BEFORE inserting any subsequent clip on that track — or the ramped clip is placed on a dedicated track. Eased curves approximate via ≥10 sub-segments per pair.
