...
4. **Media taps.** Reuse Phase 45's `ProgramCompositor` output. The composited `CVPixelBuffer` feeds an `RTCVideoSource` via a custom `RTCVideoCapturer` subclass. **Audio is harder than the symmetric "use `RTCAudioSource`" claim might suggest:** GoogleWebRTC's Swift/Obj-C SDK does NOT expose a public push-PCM API on `RTCAudioSource` / `RTCAudioTrack`. Audio enters the WebRTC engine through an `AudioDeviceModule` (ADM) — a C++ interface the engine pulls from. Our master-bus audio path therefore needs a custom C++ ADM subclass that bridges `AVAudioEngine` output into the WebRTC engine's pull model. We ship one minimal ADM (~200 lines of C++) wrapped behind a Swift facade and route master-bus samples into it via an `AVAudioSinkNode`. This is documented as the unavoidable interop cost of using GoogleWebRTC; any RTCAudioDeviceModule-named symbol referenced in earlier drafts is not part of the vanilla GoogleWebRTC public surface.
