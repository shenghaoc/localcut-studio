...
The WHIP HTTP client, settings storage, program-frame sink plumbing, WebRTC package linkage, capture-side audio bridge, and MediaMTX integration harness are implemented. The default app/test build enables the dependency with `LOCALCUT_ENABLE_WEBRTC`; custom builds can remove that flag and the package product to compile the reduced publish UI. Local `xcodebuild test` passed with the MediaMTX suite disabled by default because this machine has no Docker/Podman runtime. CI runs `Scripts/run-mediamtx-whip-integration.sh`, which starts MediaMTX, enables the test suite with `LOCALCUT_RUN_MEDIAMTX_INTEGRATION=1`, and fails if the required container runtime is missing.
