...
> AVAudioEngine device-module input hook. `LocalCutPlatform` requires WebRTC on
> macOS; Linux excludes the whole platform target. Local `xcodebuild test` passes with the
...
> binary download and shared cache without a script, symlink, or submodule. All Xcode CI test
...
- [x] **T1.2** Keep WebRTC confined to the macOS-only platform graph so Linux builds require neither the binary nor a substitute implementation.
...
- [x] **T7.4** `xcodebuild` (Debug, macOS) green for the required WebRTC-enabled app path, with Xcode per-test timeouts enabled on the full suite and focused MediaMTX integration run.
