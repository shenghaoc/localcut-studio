...
- [x] **T2.6** Overlay-only and tail intervals extend the video composition with filler frames.
...
- [x] **T3.5** Regression tests: source-local look keyframes, negative vignette, overlay transform, overlay-only duration, path validation, overlay selection reset, import filtering, LUT-only exportability, decoded clamping, and single-file bundled-overlay preservation.
