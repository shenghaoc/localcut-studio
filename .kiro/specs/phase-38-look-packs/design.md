...
4. **Scheduling and transforms.** Overlay clips are scheduled independently of media clips. Overlay-only projects and tails beyond the last video clip reuse the caption-tail filler track so AVFoundation schedules compositor frames over the overlay interval. Overlay scale is source-natural (`1.0` means original pixel size), rotation is around the source centre, and position is authored as a normalized offset from render centre. Preview and export each register overlay sources in a session-scoped compositor registry that is released with the preview item or export job.
