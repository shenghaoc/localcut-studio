...
- [ ] **T1.2** Tier B: Core ML MODNet loader with manifest + SHA-256; `URLSession` download with progress.
- [ ] **T1.3** Optional RVM (recurrent) loader with per-clip session + reset policy.

## Pipeline

- [ ] **T2.1** `IOSurface` `CVPixelBuffer` → Vision / Core ML → `CVPixelBuffer` alpha → `MTLTexture` route.
...
- [ ] **T3.1** Probe-driven `MLComputeUnits` selection.
