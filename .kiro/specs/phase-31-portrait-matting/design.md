...
1. **Model tiering.**
   ...
   - **Tier B (optional higher quality):** a Core AI port of MODNet or RVM, ~26 MB on disk. The same model is shared between preview and export via Core AI's Metal-backed feature provider; predictions run on the Neural Engine by default.
2. **Zero-copy pipeline.**
   ```
   CMSampleBuffer → CVPixelBuffer (IOSurface-backed)
                  ↓
              Vision request OR Core AI predict
                  ↓
              CVPixelBuffer alpha (single channel) → MTLTexture
                  ↓
              Compositor matte / blur passes (CIKernel or Metal compute)
   ```
   No CPU pixel round-trip. `CVPixelBuffer` from the asset reader's `IOSurface` is directly fed to Vision/Core AI; the alpha output is sampled into the compositor's Metal-backed `CIContext`.
...
6. **Capability gating.** Core AI compute-unit selection is driven by the probe: Neural Engine preferred on Apple Silicon, GPU + CPU on Intel. Hosts that can't sustain realtime at proxy resolution see an explicit "export-only" downgrade rather than a silent hang.

## Trade-offs

- Vision's built-in segmentation is the no-download winner; MODNet/RVM via Core AI buy noticeable edge quality on hair / wisps but require a Tier B download.
...
- Core AI on Intel Macs without a discrete GPU is much slower; the probe gates UX accordingly.
