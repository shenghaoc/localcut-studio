...
- The "include in bundle" vs "external-only" choice is recorded explicitly on the runtime model via `MediaItem.wantsBundling: Bool` (default `true`). This is distinct from `bundleRelativePath == nil`, which simply means "not yet placed in any bundle" — saving a project freshly-imported as `.lcbundle` flips every item whose toggle stayed on to the canonical `assets/<id>.<ext>`. The media-bin import toggle passes `wantsBundling = false` for items the user wants to keep external.
- **Current behaviour for mixed bundles:** In a project mixing bundled and external-only media, refs opened from old documents with no explicit opt-out still default to `wantsBundling = true`, so the next bundle save copies those files into `assets/`. Newly imported files respect the media-bin copy toggle.
