...
- [x] **T1.3** LRU cache backed by a dictionary + linked list under `OSAllocatedUnfairLock`; cap = 128 entries; O(1) LRU touch on lookup.
...
- [x] **T2.3** Unit test: inserting beyond the cap evicts the least-recently-used entry, and cache hits refresh LRU order.
