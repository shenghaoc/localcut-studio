...
2. **Own-app event log.** When recording is targeted at LocalCut Studio itself (own-process, detectable), we install a session-scoped event tap that logs `(time: CMTime, kind: enum { mouseDown, mouseUp, scroll, key }, position: CGPoint)` to a sidecar `.events.json` next to the chunked recording. This works only on own-process input — cross-application cursor tracking is impossible without OS-level entitlements we won't request.
