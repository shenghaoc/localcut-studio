...
2. **Own-app event log.** When recording is targeted at LocalCut Studio itself (own-process, detectable), `NSEvent.addLocalMonitorForEvents(matching:handler:)` returns an opaque monitor token; we store it on the recording session and pass it to `NSEvent.removeMonitor(_:)` on session stop AND in the session's `deinit` (belt-and-braces). Without removal the closure keeps receiving events after the session ends, retaining the sidecar writer and double-logging on the next session. The log writes `(time: CMTime, kind: enum { mouseDown, mouseUp, scroll, key }, position: CGPoint)` to a sidecar `events.json` next to the recording. Local monitors see only events delivered to our own app and do NOT need Accessibility permission — that's the deliberate restriction we want. Cross-application cursor tracking would require a `CGEventTap` with Accessibility permission, which is out of scope.
