...
- Pause/resume manifest writes are treated as critical. Failed `source-ended`, `pause`, or `resume` records keep the manifest unfinalized so crash recovery can probe fragmented chunk files, including the currently open resumed chunk.
