...
3. **Mid-session source switching.** The user can swap a display / window / app target during record. ScreenCaptureKit supports `SCStream.updateConfiguration(_:)` for the resolution / fps fields and `SCStream.updateContentFilter(_:)` for the target. We do this in-place without closing the writer, preserving timeline continuity.
...
5. **Floating control strip.** A separate `NSPanel`-class window (non-activating) that floats above all apps with start / stop / pause / source indicators / level meter. Closed → returns to the main-window control bar.
