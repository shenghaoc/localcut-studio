...
4. **Marker integration.** When analysis completes, populate the `TimelineMarker` collection with `kind: .beat`, drawn as thin verticals on the ruler with a distinct colour. Beat markers don't render in export.
