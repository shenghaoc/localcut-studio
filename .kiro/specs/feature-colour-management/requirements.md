...
- **R3.3** Sampling uses `CIFilter.areaHistogram` for the waveform columns and `CIFilter.areaAverage` for the vectorscope cells.
- **R3.4** `ScopesView` renders the latest sample with SwiftUI `Canvas`. It sets `enabled = true` on appear and `false` on disappear.
...
- **R6.3** Unit test: `ScopeSampler.sample(image:context:colorSpace:)` over a non-black `CIImage` produces a waveform with at least one column with a non-zero bin.
- **R6.4** Existing test count does not regress. `xcodebuild` (Debug, macOS) is green.
