...
1. **Engine tiering.**
   ...
   - **Tier B (optional higher-accuracy download):** a Whisper Core AI model — Apple ships official Core AI conversions of Whisper Base / Small. Size: Base ≈ 145 MB on disk post-compile, Tiny ≈ 75 MB. We bundle a small (Base) by default; Tiny is an explicit "download lighter model" option.
...
3. **Worker.** A background `actor TranscriptionService` accepts `(asset, range, language?) -> AsyncThrowingStream<CaptionLine>` and runs windowed inference (~30 s with overlap stride). Output is `CaptionLine` with a `words: [WordTiming]` array. The actor isolates Core AI / Speech calls from the main actor.
...
- Apple's Speech framework integrates with no model-download UX (already on-device on macOS 26+) and is the obvious default; Whisper Core AI is the path for users who want better Mandarin / code-switching quality at the cost of a one-time download.
- Word-level timing in Apple Speech is per-segment, not per-glyph; Whisper Core AI provides token-level timings we map to words. The UI must note when word highlights are coarser (Tier A only).
...
- `SFSpeechRecognizer.requiresOnDeviceRecognition` returns `false` on older / smaller Macs. The capability probe surfaces this; on those hosts the only path is Tier B (Whisper Core AI).
- DRM-protected audio sources fail at `AVAssetReader`; we surface this explicitly.
- The Whisper Core AI download lives in the app's sandbox container; bundle export does NOT carry it (it is regenerable by re-download).
