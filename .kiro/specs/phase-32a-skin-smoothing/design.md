...
- The implemented keyframe system so `strength` can be evaluated and authored over clip-local time. Phase 32a exposes the default strength and minimal strength-keyframe controls in the inspector.
...
4. **Inspector UI.** A "Beauty" section in the inspector with a default-strength slider, an A/B bypass toggle, a preview-only "show mask" toggle, advanced mask controls, strength-keyframe controls, and accessibility labels on all controls. The slider edits `strength.defaultValue`; keyframe controls add, update, remove, and navigate strength keyframes at the selected clip's local playhead time.
...
- Full curve-graph editing belongs to the broader keyframe UX; Phase 32a ships the required inspector-level strength controls.
