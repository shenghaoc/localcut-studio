...
3. **OPFS spill (Mac equivalent).** When in-memory budget would be exceeded, oldest keyframe-aligned spans spill to a `Caches/ReplayBuffer/<session-uuid>/` directory under user-selected file access. Each spill file is a timestamped binary record; the in-memory index keeps pointers. Saves reading from spilled chunks just `fread` them.
