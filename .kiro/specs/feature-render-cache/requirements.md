...
> Status: **Proposed**. P19 native equivalent; prerequisite for Phase 35 (speed ramps) and Phase 37 (frame interpolation).

## R1 — Composite key

- **R1.1** A `RenderCacheKey` identifies one cached post-effect-chain frame by `(clipID: UUID, effectChainHash: Int, time: CMTime, renderSize: CGSize)`.
...
- **R5.1** `EffectCompositor.applyEffectChain` consults the cache before executing the CIFilter chain.
