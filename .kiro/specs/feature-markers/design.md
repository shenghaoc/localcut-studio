...
| `Shift+M`       | Open a rename popover anchored to the selected marker, or report status guidance when no marker is selected. |
...
marker on the ruler first instead of silently no-oping.
