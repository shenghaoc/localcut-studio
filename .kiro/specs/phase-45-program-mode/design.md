...
8. **Layout track landing.** On stop: read `scene-switch` records in order, partition the session into segments, emit a `LayoutClip` per segment with `sceneSnapshot`, place on a new `TimelineTrack { type: .layout }`. `MediaItem.captureSourceID` preserves the capture-source → media mapping so `CompositionBuilder` can match scene-layer source refs to ISO track MediaItems. Re-export drives the same compositor with the same `SceneDefinition`s — the live mix replays identically.
