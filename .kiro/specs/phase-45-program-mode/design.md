...
7. **Scene definitions.** `SceneDoc` with `sceneSchemaVersion`, list of `SceneDefinition { id, name, hotkey, layers }`. Each `SceneLayer { sourceRef, transform, visible, zIndex }`. Scene geometry persists in `ProjectDoc`; device bindings (which physical device backs which `sourceRef`) live in app-local settings.
