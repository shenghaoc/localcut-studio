...
2. **LiveComposeTap.** Per-source bridge from each `TrackPipeline`'s frame reader to the compositor. Sources are `IOSurface`-backed, so the "clone" is a **zero-copy `CVPixelBuffer` wrapper** around the same underlying `IOSurface` (`CVPixelBufferCreateWithIOSurface`) — NOT a `vImageBuffer_Copy` / `memcpy` of pixels, which would saturate memory bandwidth on 4K multi-source sessions. Both the encode path and the compose path retain references to the same `IOSurface`; the source's frame reader and the compositor consume it concurrently with no extra copy. The tap is the lifecycle gate: active taps pass the same wrapper through, disposed taps drop late frames. `ProgramCompositor` is the single owner of the latest per-source frame cache, so invisible sources stay warm without duplicating buffer-retention state in the tap.
...
5. **Encoder budget.** Shared `EncoderBudget` actor with `acquire(EncoderConsumer)` → `EncoderLease | nil`. Consumers: `.export`, `.isoRecord`, `.whipPublish`, `.programIso`. Default 4 concurrent video sessions on hardware encode (enough for the 2-cam + 1-screen multi-cam smoke test alongside an export or publish session), 1 on software-only. Per-host probing can raise or lower this at runtime — VideoToolbox doesn't publish a session-count limit, so the default is conservative against real chip-family ceilings (M2/M3 Pro+ have headroom for more). Acquire all video leases up front before any source starts; on partial failure release everything and surface `ProgramSessionError.budgetExhausted`, not the lower-level `EncoderBudgetError`.
6. **Manifest extension.** Phase 41's NDJSON manifest gains TWO new record kinds. (a) `{ kind: "scene-doc", scenes: SceneDoc, atUs }` — written at session start with the full `SceneDoc` snapshot, and re-written whenever the user edits scenes mid-session. Recovery uses the LATEST `scene-doc` record found before the crash, NOT the current user's scenes (which may have been edited after the crash). (b) `{ kind: "scene-switch", sceneId, atUs }` — every switch. Layout-track reconstruction joins the two: look up each `scene-switch.sceneId` in the most-recent `scene-doc` snapshot that precedes it. Parsers that don't recognise either kind skip them (forward-compatible).
7. **Scene definitions.** `SceneDoc` with `sceneSchemaVersion`, list of `SceneDefinition { id, name, hotkey, layers }`. Each `SceneLayer { sourceRef, transform, visible, zIndex, opacity }`; `sourceRef` can target a capture source, still, or solid colour. Scene geometry persists in `ProjectDoc`; device bindings (which physical device backs which `sourceRef`) live in app-local settings. **Forward migration:** mirrors the Phase 30 `CaptionPresetV1` pattern — every read passes through a `migrateSceneDoc(_:)` step that detects the version, applies a chain of `V1→V2→…` upgrades to produce the current shape, and is the single place new schema versions get added. New fields default-fill rather than fail the load; removed fields are ignored on read and dropped on next save.
8. **Layout track landing.** On stop: read `scene-switch` records in order, partition the session into segments, emit a `LayoutClip` per segment with `sceneSnapshot`, place on a new `TimelineTrack { type: .layout }`. `MediaItem.captureSourceID` preserves the capture-source → media mapping so `CompositionBuilder` can match scene-layer source refs to ISO track MediaItems. Re-export drives the same compositor with the same `SceneDefinition`s, including synthetic solid-colour scene layers — the live mix replays identically.
...
- Scene-switch one-frame invariant requires the compositor to be one tick per output frame, with the new uniforms picked up immediately. The frame tap lifecycle discipline (dispose exactly once and drop late frames) is the safety net.
