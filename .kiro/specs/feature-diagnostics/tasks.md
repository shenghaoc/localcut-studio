...
- [x] **T1.2** `DiagnosticsAgent` (`DiagnosticsAgent.swift`) — `@Observable @MainActor`; 1 Hz `Timer`; CPU via `proc_pidinfo(getpid(), PROC_PIDTASKINFO)` (covers live + terminated threads in one libproc call, no entitlements, no deprecated Mach surface); GPU as `mean(render) / frameDuration` proxy, zeroed on idle ticks via `totalFrameCount` delta; p95 over the ring; tick emits an `os_log` event with subsystem `"com.localcutstudio.diagnostics"`.
- [x] **T1.3** Probe lifetime: `EditorModel` owns the agent; `View ▸ Show Diagnostics` toggle drives `start()` / `stop()`; the agent's `deinit` invalidates the timer **and** closes the bridge gate so a torn-down editor session doesn't leak either. `start()` preserves decoder count, seeds the drop baseline from `droppedFramesProvider()`, opens the bridge gate; `stop()` closes the gate.
