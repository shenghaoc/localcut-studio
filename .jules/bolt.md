...
**Learning:** Drawing many individual paths using multiple `context.stroke` calls inside loops in a SwiftUI Canvas (like waveform graticules and speed curve grid lines) leads to significant CPU overhead.
**Action:** Always batch related Canvas drawing operations into a single `Path` instance. Accumulate lines and shapes inside the loop, and perform a single `context.stroke` or `context.fill` call outside the loop to drastically reduce core graphics call overhead. Only batch elements that share identical styling (e.g. same color and line width).
