...
**Action:** Separate static background elements (graticule lines, target boxes, text labels) and dynamic trace elements into separate `Canvas` views layered within a `ZStack`. Ensure the background `Canvas` does not capture the rapidly changing variable, allowing it to render once and avoid per-frame text rendering costs.
