...
**Learning:** Drawing multiple independent paths using `context.fill` and `context.stroke` inside a loop for each safe zone region causes O(N) core graphics overhead. Additionally, separate translucent paths compound their alpha where they overlap, which visually breaks a unified mask.
**Action:** Always batch Canvas drawing operations. Accumulate `Path` objects inside loops (e.g., adding subpaths for each region) and issue single `context.stroke` and `context.fill` calls at the end to drastically reduce the number of render calls and preserve correct alpha blending.
