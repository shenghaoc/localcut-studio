...
**Learning:** Creating a new `Path` and executing a `GraphicsContext.fill` call for every single data point in a dense visualisation (like a waveform or vectorscope with thousands of points) causes significant CPU overhead during 30fps/60fps redraws. Naively merging every vectorscope ellipse into one path also flattens overlapping alpha — solid colours and skin-tone clusters stop reading as denser.
**Action:** Always batch Canvas drawing. For properties that vary (waveform intensity, vectorscope hit density), quantise into a small number of opacity buckets (e.g. 64) and fill once per bucket. For vectorscope, first grid-bin plot space at the ellipse diameter, count hits per cell, and map count through source-over accumulation `1 - (1 - base)^N` before bucketing so density is preserved without thousands of fills.
...
## 2026-10-25 - Batch Canvas drawing in Scopes and SpeedCurveEditor
**Learning:** Drawing many individual paths using multiple `context.stroke` calls inside loops in a SwiftUI Canvas (like waveform graticules and speed curve grid lines) leads to significant CPU overhead. Moving `stroke` outside a loop that also draws labels inverts z-order if labels stay inside the path-accumulation loop — full-width grid lines then paint over IRE labels.
**Action:** Always batch related Canvas drawing operations into a single `Path` instance. Accumulate lines and shapes inside the loop, and perform a single `context.stroke` or `context.fill` call outside the loop to drastically reduce core graphics call overhead. Only batch elements that share identical styling (e.g. same color and line width). When a loop also draws text/labels that must sit on top of those strokes, stroke the batched path first, then draw labels in a second pass.
