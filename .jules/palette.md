...
## 2026-07-15 - Add missing labels to icon-only buttons in property editors
**Learning:** Buttons containing only  may still only render as icon-only in certain SwiftUI contexts and lack proper VoiceOver labels if the visual text is omitted or ignored by the view hierarchy. In  and , the Add/Update keyframe buttons used a  but were missing explicit  and  modifiers compared to neighboring buttons.
**Action:** Always append explicit  and  modifiers to any button functioning as an icon-only control, even if its content is a  with text, to guarantee VoiceOver coverage and tooltips.
