...
**Action:** Replaced the plain `ContentUnavailableView` initialization with the trailing closure syntax to provide an `actions` block containing an 'Import Media…' CTA button. Added `.accessibilityHidden(true)` to the button because the parent "Preview" element already conveys the empty state through its `accessibilityValue`, and the import action remains available via File ▸ Import… (⌘I). Scoped `.foregroundStyle(.secondary)` to the title and description only — not the actions container — so the `.borderedProminent` button retains its default high-contrast label.
