...
## 2026-07-20 - Prevent Keychain Secrets from Syncing to iCloud
**Vulnerability:** Saved Keychain items (like authentication tokens or passwords) were using `kSecAttrAccessibleWhenUnlocked` without `kSecUseDataProtectionKeychain`. On macOS that can leave items in the file-based keychain where accessibility is ignored, and non-`ThisDeviceOnly` items may sync via iCloud Keychain.
**Learning:** Restrict credentials to the authenticating device and store them in the data-protection keychain; re-save legacy items on load so existing remembered tokens pick up the tighter policy.
**Prevention:** Always use `kSecAttrAccessibleWhenUnlockedThisDeviceOnly` with `kSecUseDataProtectionKeychain: true` when storing sensitive tokens unless cross-device syncing is an explicitly required feature.
