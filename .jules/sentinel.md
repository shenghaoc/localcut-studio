...
**Vulnerability:** Untrusted external media file metadata like `duration` was being loaded directly into internal properties without sanitization in `BeatTools.swift` and `SilenceDetector.swift`. Sanitizing only the capacity-reservation duration still left `AVAssetReader` configured with unbounded ranges and read loops that could fully decode multi-hour/hostile assets into memory.
**Learning:** Capacity reservation is not a decode budget. Metadata sanitization must also bound the reader time range *before* `startReading`, and the sample loop must hard-cap sample count so actual long media cannot exhaust memory even when reported duration is plausible.
**Prevention:** Use `.sanitized` on metadata and `CMTimeRange.sanitized(maxDurationSeconds:)` for decode windows; enforce matching max-sample budgets in read loops for beat analysis (1h) and silence detection (2h).
## 2026-07-22 - Untrusted Media Metadata Validation in ReplayBuffer
**Vulnerability:** Untrusted external media file metadata like `naturalSize`, `preferredTransform`, and `duration` was being loaded directly into internal properties without sanitization in `EditorModel+ReplayBuffer.swift`. This could lead to downstream rendering issues or DOS via extreme dimensions, transforms, or durations.
**Learning:** All media metadata must be considered untrusted and must be validated and clamped before use, even if loaded via seemingly safe APIs like `load(.duration)` and `load(.naturalSize)`.
**Prevention:** Always append `.sanitized` to `duration`, `naturalSize`, and `preferredTransform` values loaded from video tracks, as done in `DocumentController.swift` and `EditorModel+Capture.swift`. Sanitize `Clip` duration and track-collision calculations that use metadata-originated durations.
