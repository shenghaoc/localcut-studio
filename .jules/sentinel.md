...
**Vulnerability:** Untrusted external media file metadata like `naturalSize` and `preferredTransform` was being loaded directly into internal properties without sanitization in `EditorModel+ReplayBuffer.swift`. This could lead to downstream rendering issues or DOS via extreme dimensions or transforms.
**Learning:** All media metadata must be considered untrusted and must be validated and clamped before use, even if loaded via seemingly safe APIs like `load(.naturalSize)`.
**Prevention:** Always append `.sanitized` to `naturalSize` and `preferredTransform` values loaded from video tracks, as done in `DocumentController.swift` and `EditorModel+Capture.swift`.
